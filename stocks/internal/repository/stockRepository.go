...
type StockRepoInterface interface {
	...
	GetBySku(ctx context.Context, id models.SKUID) (*GetSKU, error)
}
...
func (r *StockRepo) GetBySku(ctx context.Context, id models.SKUID) (*GetSKU, error) {
	query := `SELECT name, price, count FROM sku WHERE sku_id = $1`

	var item GetSKU
	err := r.tx.QueryRow(ctx, query, id).Scan(&item.Name, &item.Price, &item.Count)
	if err != nil {
		return nil, err
	}
